...
int[][] grid = strGrid.Replace("\r", "").Split('\n').Select(n => n.Split(' ').Select(n => Convert.ToInt32(n)).ToArray()).ToArray();
...
for (int i = 0; i < 20; i++) // row
{
    for (int j = 0; j < 20; j++) // column
    {
        ...
        // diagonal
        if (i < 17 && j < 17) // to bottom-right
        {
            int sum = grid[i][j] * grid[i + 1][j + 1] * grid[i + 2][j + 2] * grid[i + 3][j + 3];
            max = sum > max ? sum : max;
        }
        if (i < 17 && j > 2) // to bottom-left
        {
            int sum = grid[i][j] * grid[i + 1][j - 1] * grid[i + 2][j - 2] * grid[i + 3][j - 3];
            max = sum > max ? sum : max;
        }
        if (i > 2 && j < 17) // to top-right
        {
            int sum = grid[i][j] * grid[i - 1][j + 1] * grid[i - 2][j + 2] * grid[i - 3][j + 3];
            max = sum > max ? sum : max;
        }
        if (i > 2 && j > 2) // to top-left
        {
            int sum = grid[i][j] * grid[i - 1][j - 1] * grid[i - 2][j - 2] * grid[i - 3][j - 3];
            max = sum > max ? sum : max;
        }
...
Console.WriteLine("Maximum product in the grid {0}", max); //70600674